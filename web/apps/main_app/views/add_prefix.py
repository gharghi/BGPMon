...
from web.apps.main_app.forms import AddPrefixForm
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if form.is_valid():
                ...
                return render(request, 'add_prefix/make_policy.html',
                              {'route_objects': route_objects, 'saved_prefixes': saved_prefixes})
    ...
    try:
        ...
        if prefix:
            ...
            if request.user == prefix.user:
                ...
                return render(request, 'add_prefix/prefix_policy.html',
                              {'origins': origins, 'prefix': prefix, 'saved_origins': saved_origins})
        else:
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if form.is_valid():
                username = request.user
                prefix = request.POST.getlist('prefix')
                remaining_prefix_limit = user_limit(request)['prefix'] - Prefix.objects.filter(user=request.user).count()
                if remaining_prefix_limit < 0 and remaining_prefix_limit < len(prefix):
